...
function buildDriverSelect(orderId, currentDriverId, currentDriverName) {
  // Solo mostrar opciones con drivers que est??n Available o Busy (activos)
  ...
  // Si hay un driver asignado que est?? offline, igual mostrarlo como opci??n
...
function renderOrders(orders) {
  ...
  if (!orders.length) {
    tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state"><p>No hay pedidos todav??a</p></div></td></tr>';
    return;
  }
...
async function loadDrivers() {
  try {
    ...
    _cachedDrivers = drivers; // guardar para dropdown de asignaci??n
...
async function assignDriverToOrder(orderId, driverId) {
  try {
    if (!driverId) {
      toast('Selecciona un domiciliario v??lido', 'error');
...
function downloadDailyReport() {
  ...
  var rows = [['C??digo', 'Cliente', 'Tel??fono', 'Direcci??n', 'Domiciliario', 'Estado', 'Hora']];
  var todayStr = new Date().toISOString().slice(0, 10);
  var todayOrders = _cachedOrders.filter(function (o) {
    return o.created_at && o.created_at.slice(0, 10) === todayStr;
  });
  if (!todayOrders.length) { toast('No hay pedidos del d??a para exportar', 'error'); return; }
...
async function clearTodayOrders() {
  if (!confirm('??Eliminar todos los pedidos de hoy? Esta acci??n no se puede deshacer.')) return;
  try {
...
function setupAddressAutocomplete() {
  ...
  // Bias hacia Soledad/Barranquilla Atl??ntico
  ...
  autocomplete.addListener('place_changed', function () {
    ...
    if (!place.geometry || !place.geometry.location) {
      toast('No se encontr?? ubicaci??n exacta. Por favor escribe de nuevo o s?? m??s espec??fico.', 'error');
      return;
    }
    _addrLat = place.geometry.location.lat();
    _addrLng = place.geometry.location.lng();
    toast('??? Ubicaci??n confirmada');
  });
...
form.addEventListener('submit', async function (e) {
  ...
  if (!_addrLat || !_addrLng) {
    toast('Por favor, selecciona una direcci??n v??lida del autocompletado antes de crear el pedido', 'error');
    return;
  }
...
// No llamamos a refreshAll() directamente aqu?? afuera; la llamamos cuando Google Maps est?? listo (arriba) y cada 5s
